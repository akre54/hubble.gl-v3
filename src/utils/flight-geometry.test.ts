...
describe('generateRoutePath function', () => {
  it('generates a route path with correct segment count', () => {
    const path = generateRoutePath({
      origin: { lat: 0, lng: 0, alt: 0 },
      destination: { lat: 10, lng: 10, alt: 10 },
      arcHeight: 100,
      segmentCount: 5,
    })
  ...
  it('generates a route path with correct apex height', () => {
    const path = generateRoutePath({
      origin: { lat: 0, lng: 0, alt: 0 },
      destination: { lat: 10, lng: 10, alt: 0 },
      arcHeight: 100,
      segmentCount: 5,
    })
  ...
  it('routes work', () => {
    const path = generateRoutePath({
      origin: { lat: 40.72145, lng: -73.9974, alt: 0 },
      destination: { lat: 40.655, lng: -73.77585, alt: 0 },
      arcHeight: 10000,
      segmentCount: 11,
      tilt: 0
    })
