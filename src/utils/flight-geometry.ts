...
export const generateRoutePath = ({
  ...
  // TODO: Add hover height somewhere

  const altSteps = segmentRatios(segmentCount, smoothHeight)
  const alts = altSteps.map(altStep => paraboloid(arcHeight, origin.alt, destination.alt, altStep))

  const positionSteps = segmentRatios(segmentCount, smoothPosition)

  // const lngs = mixspace(origin.lng, destination.lng, positionSteps)
  // Calculate the shortest path longitude interpolation
  let lngs: number[]
  if (wrapLongitude) {
    const diff = destination.lng - origin.lng
    const deltaLng = diff > 180 ? diff - 360 : diff < -180 ? diff + 360 : diff // Adjust for anti-meridian crossing
    lngs = positionSteps.map(step => origin.lng + deltaLng * step)
    // Normalize longitudes that wrap around
    lngs = lngs.map(lng => (lng > 180 ? lng - 360 : lng < -180 ? lng + 360 : lng))
  } else {
    lngs = mixspace(origin.lng, destination.lng, positionSteps)
  }
  const lats = mixspace(origin.lat, destination.lat, positionSteps)

  const points = alts.map((alt, idx) => [lngs[idx], lats[idx], alt])

  return _tilt === 0
    ? points
    : points.map(p => tiltPoint(p, origin, destination, _tilt))
}

export const tiltPoint = (point: number[], origin: Point, destination: Point, tilt: number) => {
  ...
  const dirX = destination.lng - origin.lng
  const dirY = destination.lat - origin.lat
